...
export function generate(telemetry: fmxxxx.Telemetry, timestamp: Date): api.v2.InfoEvent[] {
	// in case of partial frame, generic event properties are generated separately of telemetry-specific
	type PartialInfoObject = {
		[name in keyof api.v2.InfoEvent]?: api.v2.InfoEvent[name];
	};
	...
	if (telemetry.permanent) {
		for (const name of Object.keys(telemetry.permanent)) {
			const item: fmxxxx.TelemetryItem = telemetry.permanent[name];
			switch (name) {
	...
	if (telemetry.obd) {
		for (const name of Object.keys(telemetry.obd)) {
			const item: fmxxxx.TelemetryItem = telemetry.obd[name];
			switch (name) {
	...
	// eventual iteration
	if (telemetry.eventual) {
		for (const name of Object.keys(telemetry.eventual)) {
			const item: fmxxxx.TelemetryItem = telemetry.eventual[name];
			switch (name) {
				case 'Alarm':
					events.push({
						name: 'alarm',
